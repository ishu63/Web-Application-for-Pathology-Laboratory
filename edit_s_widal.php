...
$sql1="SELECT * FROM s_widal WHERE P_ID = '$row[0]'";
...
<div class="bg-main">
    ...
    <a class="btn">Send</a>
		...
		-->
			<center><h1>S.WIDAL TEST</h1></center>
			<form id="form2">
				<div class="grid_8">
			...
			<form id="form1" action="edit_s_widal1.php?P_ID=<?php echo $row[0];?>" method="post">
				<div class="grid_24">
					<?php
						$sql1="SELECT * FROM s_widal WHERE P_ID = '$row[0]'";
						$result1=mysql_query($sql1) or die('mysql error');
					...
					?>
					<label><span class="text-form">Reported On:</span><input type="date" value="<?php echo $row1[1];?>" name="Reported_On"></label>
					<u><h4>ANTIGEN EMULSION</h4></u>
					<label><span class="text-form">Method:</span><input type="text" value="TUBE" name="Method"></label>
					<label><span class="text-form">S. Typhi 'O':</span><input type="text" name="o" value="<?php echo $row1[2];?>"></label>
					<label><span class="text-form">S. Typhi 'H':</span><input type="text" name="h" value="<?php echo $row1[3];?>"></label>
					<label><span class="text-form">S. Paratyphi 'AH':</span><input type="text" value="<?php echo $row1[4];?>" name="ah"></label>
					<label><span class="text-form">S. Paratyphi 'BH':</span><input type="text" value="<?php echo $row1[5];?>" name="bh"></label>
					<label><span class="text-form">Result:</span>
						<div class="wrapper"><input type="radio" name="reasult" value="0" <?php if($row1[6]==0){echo "checked";}?>><div class="text-form2 fleft">Negative</div></div>
						<div class="wrapper"><input type="radio" name="reasult" value="1" <?php if($row1[6]==1){echo "checked";}?>><div class="text-form2 fleft">Positive</div></div>
						<div class="wrapper"><input type="radio" name="reasult" value="2" <?php if($row1[6]==2){echo "checked";}?>><div class="text-form2 fleft">AMNESTIC REACTION</div></div>
					</label>
				</div>
